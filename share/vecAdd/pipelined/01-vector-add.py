...
@triton.autotune(
    configs=[
        triton.Config({'BLOCK_SIZE': 2048, 'stride': 512}, num_stages=8, num_warps=1),
    ],
    key=[],
)
@triton.jit
...
def add(x: torch.Tensor, y: torch.Tensor):
    ...
    kernel_info = add_kernel[grid](x, y, output, n_elements)
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        x_vals=[2**27],  # Different possible values for `x_name`.
